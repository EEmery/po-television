...
def wave(step_num, sound_level, prev_step_sound_avg, prev_step_sound_max, memory):
    """Iterates over all the frames in each new step, forming a wave animation. The animation can
    contain any number of frames, it doesn't have to have exactly 16 frames. This function can be
    copied without changes to build other simple animations that just iterates over hard coded
    frames.
